"""
Salp Chain MAPPO — MAXIMUM PARALLELISM EDITION
===============================================
MAPPO (Multi-Agent PPO) replaces MADDPG.

Architecture changes vs MADDPG
--------------------------------
* Actor:    Gaussian policy (mean + log_std) instead of deterministic tanh.
            Separate actor per agent (parameter sharing is OFF by default,
            but easy to enable — see SHARE_PARAMS flag).
* Critic:   Centralised value function V(global_obs) shared across all agents
            (one critic net, one optimiser).  Each agent's advantage is
            computed against this shared baseline.
* Storage:  On-policy RolloutBuffer with Generalised Advantage Estimation
            (GAE-λ) instead of a replay buffer.
* Update:   PPO-clip loss for actors + value-function loss for critic +
            entropy bonus.  Multiple epochs per rollout (PPO_EPOCHS).
* Noise:    OU noise is gone; exploration comes from the stochastic policy.

Reward function (ported from MADDPG version):
  - Smooth normalised distance shaping: delta/(W+H) * 10
  - Time penalty: -0.5 / step
  - Collision penalty: -15.0
  - Wall hit penalty: -3.0
  - Success: 50 base + 200*(1 - steps/max_steps) efficiency bonus
  - Timeout: -50.0 flat
  - No reward clipping

Logging (ported from MADDPG version):
  - episode_log.csv   — every episode: reward, success, timeout, collisions,
                        steps, per-salp reward/collision/wall breakdowns
  - timestep_log.csv  — every step: all per-salp quantities

Parallelism stack (unchanged from original MADDPG version):
...
print(f"Device: {DEVICE}  |  torch.compile: {USE_COMPILE}"
      ...
      f"  |  Total envs: {TOTAL_ENVS}  |  Algo: MAPPO"
...
class FastEnv:
    ...
    # ── Reward (ported from MADDPG version) ───────────────────────────────────
...
# =============================================================================
# LOGGING HELPERS  (ported from MADDPG version)
# =============================================================================
...
class EpisodeLogger:
    ...
    def log(self, episode_idx: int, r: dict):
        row = {
            ...
            "collisions": r["collision_sum"],   # full episode total
...
def analyze_and_plot() -> None:
    ...
    fig.suptitle(f"Salp MAPPO — {TOTAL_ENVS} envs ({NUM_WORKERS} workers)")
    w = 100
...
def _run_training_loop(
    ...
    pbar = tqdm(
        total=max_episodes,
        initial=start_ep,
        desc=f"MAPPO ({TOTAL_ENVS} envs, {DEVICE})",
        dynamic_ncols=True,
    )
