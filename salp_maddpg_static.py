...
Changes from original:
  ...
  - Static obstacles: land polygons generated once per env, never rebuilt
  - Speed optimisations:
    * Precomputed polygon edge arrays and bounding boxes (no np.roll per call)
    * Bbox early-exit in all collision and observation queries
    * Vectorised _wall_bounce, _enforce_rigid_links (no Python loops)
    * Batched OU noise sampling across envs (one np call per agent)
    * Faster replay buffer sampling via np.random.randint
    * ENVS_PER_WORKER raised to 4 to reduce IPC overhead
"""
...
ENVS_PER_WORKER  = 4          # more envs/worker → fewer IPC round-trips
...
# Paths
SAVE_DIR          = "salp_saves_maddpg_static"
os.makedirs(SAVE_DIR, exist_ok=True)
CHECKPOINT_PATH   = os.path.join(SAVE_DIR, "maddpg_checkpoint.pt")
EPISODE_LOG_PATH  = os.path.join(SAVE_DIR, "episode_log.csv_maddpg_static")
TIMESTEP_LOG_PATH = os.path.join(SAVE_DIR, "timestep_log_maddpg_static.csv")
...
class ReplayBuffer:
    ...
    def _sample_tensors(self, batch_size):
        # randint is ~10x faster than np.random.choice(..., replace=False)
        # for large buffers; tiny duplicate probability is fine in practice
...
class MADDPG:
    ...
    def act_batch(self, all_env_obs: List[List[np.ndarray]], explore: bool = True) -> List[List[np.ndarray]]:
        ...
        Noise is sampled in one batched numpy call per agent instead of looping per env.
        """
        ...
        for i in range(N_SALPS):
            ...
            if explore:
                # Update each noise state and collect; avoids Python loop overhead
                # by batching the OU update across all envs for this agent
                sigma = self.noise_sigma
                theta = OU_THETA
                states = np.stack([self.noises[e][i].state for e in range(n_envs)])  # (n_envs, 2)
                noise  = theta * (- states) + sigma * np.random.randn(n_envs, ACTION_DIM).astype(np.float32)
                states += noise
...
class FastEnv:
    ...
    def __init__(self, W=W_DEFAULT, H=H_DEFAULT, margin=MARGIN_DEFAULT,
        ...
        # Precomputed edge data for each polygon (set in _build_land):
        #   _poly_edges_a[k]  — vertices a  (N_edges, 2)
        #   _poly_edges_ab[k] — edge vectors (N_edges, 2)
        #   _poly_edges_ab2[k]— dot(ab,ab)   (N_edges,)
        #   _poly_bbox[k]     — (xmin,ymin,xmax,ymax) for cheap culling
    ...
    def _build_land(self):
        ...
        # Precompute per-polygon edge arrays for fast closest-point queries
        self._poly_edges_a   = []
        self._poly_edges_ab  = []
        self._poly_edges_ab2 = []
        self._poly_bbox      = []
        for poly in polys:
            a   = poly
            b   = np.roll(poly, -1, axis=0)
    ...
    def _closest_on_poly_fast(self, px: float, py: float, k: int) -> Tuple[float, float]:
        """Uses precomputed edge arrays — avoids np.roll and array alloc each call."""
    ...
    def _enforce_separation(self):
        restitution = 0.6
        # Vectorised pairwise check — N_SALPS=5 → 10 pairs
        for i in range(N_SALPS):
            for j in range(i + 1, N_SALPS):
                ...
                if dist < min_d:
                    inv  = 1.0 / dist
                    ux, uy = dx * inv, dy * inv
                    overlap = (min_d - dist) * 0.7
                    self.pos[i, 0] -= ux * overlap;  self.pos[i, 1] -= uy * overlap
                    self.pos[j, 0] += ux * overlap;  self.pos[j, 1] += uy * overlap
                    dvx = self.vel[j, 0] - self.vel[i, 0]
                    dvy = self.vel[j, 1] - self.vel[i, 1]
                    vn  = dvx * ux + dvy * uy
                    if vn < 0:
                        imp = -(1 + restitution) * vn * 0.5
                        self.vel[i, 0] -= imp * ux;  self.vel[i, 1] -= imp * uy
                        self.vel[j, 0] += imp * ux;  self.vel[j, 1] += imp * uy

    def _wall_bounce(self):
        lo   = self.margin + self.MAX_EXTENT          # (N_SALPS,)
        hi_x = self.W - self.margin - self.MAX_EXTENT
        hi_y = self.H - self.margin - self.MAX_EXTENT

        hit_lo_x = self.pos[:, 0] < lo
        hit_hi_x = self.pos[:, 0] > hi_x
        hit_lo_y = self.pos[:, 1] < lo
        hit_hi_y = self.pos[:, 1] > hi_y

        self.pos[:, 0] = np.where(hit_lo_x, lo,   self.pos[:, 0])
        self.pos[:, 0] = np.where(hit_hi_x, hi_x, self.pos[:, 0])
        self.pos[:, 1] = np.where(hit_lo_y, lo,   self.pos[:, 1])
        self.pos[:, 1] = np.where(hit_hi_y, hi_y, self.pos[:, 1])

        self.vel[:, 0] = np.where(hit_lo_x,  np.abs(self.vel[:, 0]) * 0.35, self.vel[:, 0])
        self.vel[:, 0] = np.where(hit_hi_x, -np.abs(self.vel[:, 0]) * 0.35, self.vel[:, 0])
        self.vel[:, 1] = np.where(hit_lo_y,  np.abs(self.vel[:, 1]) * 0.35, self.vel[:, 1])
        self.vel[:, 1] = np.where(hit_hi_y, -np.abs(self.vel[:, 1]) * 0.35, self.vel[:, 1])

        self.wall_flags[:] = hit_lo_x | hit_hi_x | hit_lo_y | hit_hi_y
    ...
    def _resolve_land_collisions(self):
        ...
        for i in range(N_SALPS):
            ...
            for k, poly in enumerate(self.land_polys):
                xmn, ymn, xmx, ymx = self._poly_bbox[k]
                # Cheap bbox cull before expensive polygon ops
                if px < xmn - min_sep or px > xmx + min_sep or \
                   py < ymn - min_sep or py > ymx + min_sep:
                    continue
                cx, cy = self._closest_on_poly_fast(px, py, k)
                dx, dy = px - cx, py - cy
                ...
                if inside or dist < min_sep:
                    ...
                    inv = 1.0 / dist
                    nx, ny = dx * inv, dy * inv
                    self.pos[i, 0] = cx + nx * min_sep
                    self.pos[i, 1] = cy + ny * min_sep
    ...
    def _resolve_link_collisions(self):
        for i in range(N_SALPS - 1):
            ...
            lx_min = min(pi[0], pj[0]);  lx_max = max(pi[0], pj[0])
            ly_min = min(pi[1], pj[1]);  ly_max = max(pi[1], pj[1])
            for k, poly in enumerate(self.land_polys):
                xmn, ymn, xmx, ymx = self._poly_bbox[k]
                if lx_max < xmn or lx_min > xmx or ly_max < ymn or ly_min > ymx:
                    continue
                pts = poly
                n   = len(pts)
                for j in range(n):
                    p1, p2 = pts[j], pts[(j + 1) % n]
                    if segments_intersect(pi, pj, p1, p2):
                        edge   = p2 - p1
                        ...
                        if np.dot(mid - p1, normal) < 0:
                            normal = -normal
                        for kk in (i, i+1):
                            self.pos[kk] += normal * 6.0
                            vn = np.dot(self.vel[kk], normal)
                            if vn < 0:
                                self.vel[kk] -= vn * normal

    def _enforce_rigid_links(self):
        d    = self.pos[1:] - self.pos[:-1]           # (N_SALPS-1, 2)
    ...
    def get_obs(self):
        ...
        dx_all     = gx - self.pos[:, 0]
        dy_all     = gy - self.pos[:, 1]
        dist_all   = np.hypot(dx_all, dy_all)

        obs_list = []
        for i in range(N_SALPS):
            x, y = float(self.pos[i, 0]), float(self.pos[i, 1])
            dx, dy = float(dx_all[i]), float(dy_all[i])
            dist_to_goal = float(dist_all[i]) / self.W
            ...
            if self.land_polys:
                ...
                for k in range(len(self.land_polys)):
                    xmn, ymn, xmx, ymx = self._poly_bbox[k]
                    # Quick bbox distance lower-bound to skip distant obstacles
                    bbox_dx = max(xmn - x, 0.0, x - xmx)
                    bbox_dy = max(ymn - y, 0.0, y - ymx)
                    if math.hypot(bbox_dx, bbox_dy) >= min_d:
                        continue
                    cx, cy = self._closest_on_poly_fast(x, y, k)
